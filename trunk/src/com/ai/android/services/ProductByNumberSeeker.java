...
/*
 *Class used for a Product request 
 * */
public class ProductByNumberSeeker extends GenericSeeker<Product> {
	
	
	private String url1="http://api.productwiki.com/connect/api.aspx?op=product&format=xml&idtype=";
	private String url2="&idvalue=";
	private String url3="&key=2358feba734043a9b2f6c52520f5feb5&fields=description";
	private static  String xmlUrl ;
	private final String TAG = getClass().getSimpleName();
	
	ArrayList<Product> prductList2;
	
	public ArrayList<Product> find(String query,String type) {					// perform search without limitation.
		ArrayList<Product> personList = retrieveProductsList(query,type);
		return personList;
	}
	
	public ArrayList<Product> find(String query,String type, int maxResults) {  	// limiting maximum numbers of results displaying to the user
		ArrayList<Product> personList = retrieveProductsList(query,type);
	...
	public String constructSearchUrl(String query,String type) {				// constructing the Search request- GET HTTP request according the ProductWiki API
