...
public class ProductByIDSeeker {
	...
	private String url2 = "&key=2358feba734043a9b2f6c52520f5feb5&fields=proscons,description,key_features,tags";
	...
	public String constructSearchUrl(String query) { // constructing the Search
														// request- GET HTTP
														// request according the
														// ProductWiki API
	...
	private void performSearch() { // method that doing the actual search.
		try {
			...
			product = parseFromUrl(); // The result of parseUrl is a product
										// object.
	...
	private Product parseFromUrl() throws XmlPullParserException, IOException { // method
																				// that
																				// initiate
																				// a
																				// Pullparser
	 ...
	 * number of “image” elements and number of “image” elements. So drill-down
	 * from the parent elements to the child ones using a dedicated method for
	 * the parsing of each element. From one method to an other, we pass the
	 * XmlPullParser instance as argument since there is a unique parser
	 * implementing the parsing. The result of each method is an instance of the
	 * model class and finally a list of products. In order to check the name of
	 * the current element, we use the getName method and in order to retrieve
	 * the enclosed text we use the nextText method.
	 */

	private Product parseProducts(XmlPullParser parser)
			throws XmlPullParserException, IOException {

		Product product2 = null;
		while (parser.nextTag() == XmlPullParser.START_TAG) {

			product2 = parseProduct(parser); // The result of parseProduct is an
	...
	private Product parseProduct(XmlPullParser parser)
		...
		while (parser.nextTag() == XmlPullParser.START_TAG) { // parser.nextText()
			...
			if (parser.getName().equals("id")) {
				...
				parseProscons(parser, product); // the parseProscons is a void
												// method where pros and cons
	...
	private ArrayList<Image> parseImage(XmlPullParser parser)
		...
		while (parser.nextTag() == XmlPullParser.START_TAG) {
			...
			if (parser.getName().equals("rawimage")) { // checking whether image
														// type is raw
	...
	private void parseProscons(XmlPullParser parser, Product product)
		...
		parser.nextTag();
		ArrayList<String> pros = new ArrayList<String>(); // declaring a list to
															// put all pro
															// statements of a
															// product.
		ArrayList<String> cons = new ArrayList<String>(); // declaring a list to
															// put all cons
															// statements of a
															// product.
		HashMap<String, String> prosm = new HashMap<String, String>();
		HashMap<String, String> consm = new HashMap<String, String>();
		while (parser.nextTag() == XmlPullParser.START_TAG) { // parsing start
																// TAG to
																// "statement".
			String pro = null;
			String proscore = null;

			while (parser.nextTag() == XmlPullParser.START_TAG) {

				if (parser.getName().equals("text")) {
					String text = parser.nextText();
					pro = text; // Adding each pro statement to the pros list
					pros.add(text);
					...
					proscore = score;
					prosm.put(pro, score);
					Log.d(TAG, "Start tag " + pro);
					Log.d(TAG, "Start tag " + proscore);
		...
		product.pros = pros;
		product.prosm = prosm;// adding pros list to the passed Product object
		...
		while (parser.nextTag() == XmlPullParser.START_TAG) {
			...
			String con = null;
			String conscore = null;
			while (parser.nextTag() == XmlPullParser.START_TAG) {

				if (parser.getName().equals("text")) {
					String text = parser.nextText();
					con = text;// Adding each cons statement to the pros list
					cons.add(text);
					...
					String score = parser.nextText();
					conscore = score;
					consm.put(con, conscore);
					Log.d(TAG, "cons " + con);
					Log.d(TAG, "Score " + conscore);
				} else {
					parser.nextText();
				}
			}
		...
		product.cons = cons;
		product.consm = consm;// adding cons list to the passed Product object
		parser.nextTag();
	...
	private void parseTags(XmlPullParser parser, Product product)
			throws XmlPullParserException, IOException {
		...
		while (parser.nextTag() == XmlPullParser.START_TAG) {
			...
			while (parser.nextTag() == XmlPullParser.START_TAG) {
				...
				} else if (parser.getName().equals("number_of_taggers")) {
					String score = parser.nextText();
					value = score;
					tags.put(tagname, score);
					Log.d(TAG, "Start tag " + tagname);
					Log.d(TAG, "Start tag " + value);
